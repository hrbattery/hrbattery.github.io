...
- 历史上的`inline`关键字是提示编译器可以将该函数嵌入到其他函数中
- 但现在这种用法已经过时了，**不要用`inline`关键字要求内联展开函数**。
- 在现代c++中，`inline`概念引申为**允许多重定义**。先前我们提过头文件里不要放置函数的实现，会导致违反ODR而报错，但标记成`inline`之后就可以避免这个问题，只要所有的定义都是一致的。
- 为了能够展开函数，**编译器必须能够在函数调用处看到函数的定义**。因此，**内联函数通常被定义在头文件中**，这样它们就可以被包含到使用它们的源文件中，以便编译器看到函数的完整定义。

## 6.9 - 使用 inline 变量共享全局常量

在很多应用程序中，有些符号常量需要被所有的代码使用。与其在多个文件中各定义一遍这些变量，不如将它们集中定义在一个地方然后按需使用。这样，万一你需要修改它们的值，你只需要在一处修改即可。
...
因为 const 全局变量具有**内部链接**属性，因此每个变量在.cpp文件之间是独立的，而且链接器并不能看到它们。在大多数情况下，由于它们是常量，所以编译器通常会将其“优化掉”。
...
避免这个问题的方法之一，是将全局常量定义为**外部变量**，这样一个变量（初始化一次）就可以被所有的文件“共享使用”。为此，我们必须将常量定义在一个a.cpp文件中（确保它只被定义一次），然后在头文件中添加其前向定义（该头文件仍需要被包含到使用了该变量的文件中）。

以上方法的问题是：

- 首先，这些常量现在只会在它们被定义的文件中被看做编译时常量。而在其他文件中，编译器只能看到它们的声明，而看不到常量具体的中（必须有链接器进行解析）。
  - 因此，在变量定义处以外的地方，这些常量都不能被看做是编译时常量，也就不能在要求使用编译时常量的地方使用它们。
- 另外，因为编译时常量相对于运行时常量更容易优化，所以对于这些运行时常量，编译器可能无法对其进行足够的优化。

### 将全局常量作为内联变量 (C++17)

C++17 引入了一个新的概念——**内联变量/inline variables**，这允许变量在多个文件中被多次定义。这样我们就可以沿用第一种方法、将全局常量放在头文件中，并避免了变量在多个文件中的重复定义导致的内存占用问题。

## 6.10 - 静态局部变量

此前我们提到， `static`关键字可以使全局变量具有**内部链接**的属性，那么，用在没有链接属性的局部变量上呢？

答案是会使局部变量拥有静态的存储持续时间，即其会在程序初始化时创建、第一次被定义时初始化，并在程序结束时才销毁，也就是说其值可以保留到离开作用域后。

一些注意事项：
- 静态局部变量和全局变量一样都可以持续到程序结束前，但是它的可见范围更小，也就更安全。
- 对静态局部变量进行初始化。
- 静态局部变量只会在第一次执行时被初始化，后面不会对其进行重复地初始化。
- 当变量创建或者初始化开销很大时，可以将其定义为静态const局部变量
- 除非变量永远不需要被重置，否则要避免使用静态局部变量

## 6.11 - 作用域和链接小结

你不能摘抄一篇摘抄，不如直接看原文[6.11 - 作用域和链接小结](https://hanxiaomax.github.io/Learncpp_CN/06-%E4%BD%9C%E7%94%A8%E5%9F%9F%E3%80%81%E7%94%9F%E5%91%BD%E5%91%A8%E6%9C%9F%E5%92%8C%E9%93%BE%E6%8E%A5/6-11-Scope-duration-and-linkage-summary)

## 6.12 - using 声明和 using 指令

在函数内部使用`using`声明还算安全：
...
，于函数来说，把它们定义在匿名命名空间中的效果和把它们定义为static类型的效果是完全一样的，避免逐个定义
- 定义在内联命名空间中的内容都属于其父命名空间。不过，内联命名空间并不会给其内部定义的内容赋予内部链接。